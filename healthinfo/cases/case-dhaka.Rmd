...
数据集源自于对孟加拉国Dhaka，824名年轻都市青年的调查，采用GHQ-12测量了受试者的心理健康状况，并收集了受试者的社会网络关系数据。据此，可以估计社会关系在缓解抑郁和焦虑以及促进人群的心理健康方面的作用发挥着重要作用。
...
横断面研究设计，2016年12月，针对Dhaka市Vashantek社区某一分区18岁-29岁男性进行了为期26天的访谈，收集了824名青年的心理健康状况数据，详细的社会经济学特征数据和朋友网络数据。

**数据详情**

***心理健康测量***：采用了GHQ-12测量受试者的心理健康状况，12个健康问题，四点式李克特量表法计分，分数越高表明心理健康状况不佳。量表详情与测量结果请看附件A。
...
通过询问确定了824名受试者的朋友关系，建立了824*824邻接矩阵，使用社会网络分析软件Pajek进行网络特征值的计算。因此，在数据集中直接给出了社会网络特征指标计算的结果。

为了还原整个数据分析过程，了解社会网络分析的应用，我们需要根据论文提供的社会网络特征线索，构造一个“相似”的随机网络，并从该网络中生成网络特征指标变量。

**统计分析**

为揭示社会关系与心理健康之间的关系，研究使用不同的社会网络特征指标分别运行了重线性回归模型，社会经济学特征指标将作为控制变量纳入到每个回归模型中。同时对连续型变量进行了标准化处理，使其具有可比性，符合多重线性回归应用条件。

在R语言实现的第三部分，我们将分别运行一个基准模型（因变量为社会经济学指标）和四个纳入不同解释变量（因变量分别为度及三个中心性指标）的模型。
...
第一步：读取论文数据集，应用数据集中的人口统计学特征信息、社会经济学特征信息及心理健康测量信息。

第二步：模拟论文数据构造社会网络，从既定的社会网络中生成网络指标变量。
...
构造一个尽可能“相似”的随机网络，我们需要一些线索：节点-824，朋友关系的平均数量(average no of friendship ties)-1.6
...
为了更加了解朋友网络，让我们绘制“度”的直方图，再用fruchterman-reingold算法的布局绘制随机生成的仿-824名青年朋友网络图。
...
***度（D）***指标反映受试者在朋友网络中朋友数量，还有三个中心性指标可以反映受试者在网络中的位置。

我们在第三步的分析中将用这四个指标反映朋友网络特征，需要说明的是，最后将会对三个中心性指标进行标准化处理。
...
函数lm(y ~ x1 + x2 + x3 + ... + xn)可用于拟合回归模型。揭示社会网络对心理健康的作用，我们运行了5个回归模型。

*model(1)* 基准模型，因变量为*标准化的心理健康状况值（zghq）*，控制变量为社会经济学特征指标，如*年龄（rage），职业（qa_）*等。

*model(2)* 在基准模型的基础上，加入了社会网络特征指标*度（D）*作为解释变量。

*model(3) ~ model(5)* 在基准模型的基础上，分别加入了社会网络特征指标*三个标准化的中心性指标（sCloc，sBetc，sEigc）*作为解释变量。


```{r}

# base
fit_1 <- lm(zghq ~ rage + reduc + rvas + rmar + requ
            + qa_12_1 + qa_12_2 + qa_12_3 + qa_12_4 + qa_12_5 + qa_12_6 +qa_12_20)

# degree
fit_2 <- lm(zghq ~ D + rage + reduc + rvas + rmar + requ
            + qa_12_1 + qa_12_2 + qa_12_3 + qa_12_4 + qa_12_5 + qa_12_6 +qa_12_20)


# closeness
fit_3 <- lm(zghq ~ sCloc + rage + reduc + rvas + rmar + requ
            + qa_12_1 + qa_12_2 + qa_12_3 + qa_12_4 + qa_12_5 + qa_12_6 +qa_12_20)


# betweenness
fit_4 <- lm(zghq ~ sBetc + rage + reduc + rvas + rmar + requ
            + qa_12_1 + qa_12_2 + qa_12_3 + qa_12_4 + qa_12_5 + qa_12_6 +qa_12_20)


# eigenvalue
fit_5 <- lm(zghq ~ sEigc + rage + reduc + rvas + rmar + requ
            + qa_12_1 + qa_12_2 + qa_12_3 + qa_12_4 + qa_12_5 + qa_12_6 +qa_12_20)
...
# output as a table
stargazer(fit_1, fit_2, fit_3, fit_4, fit_5, type = "html", report = "vc*")


```

模型拟合结果表明，有更多的关系或更多的朋友（p<0.05)心理健康状况将会更好，中心性的三个变量也均显著影响因变量（p<0.1,p<0.05,p<0.01），即在网络中的位置越重要，越嵌入与网络中的人心理健康状况会更好。

同时，控制变量的分析结果也很有趣，例如，出生在当地的受试者有更好的心理健康状况，受过更高教育的受试者有更好的心理健康状况，随着年龄的增长心理健康状况将会恶化。

最后，我的结果证实了社会关系将会影响人的心理健康，社会关系较好，网络嵌入较高的年轻人心理健康状况较好。
