...
**课堂思考: (1)c如何在熟悉的编程语言中，撰写函数估计多元线性模型？(2) 课堂思考: 在实践中，OLS会造成什么缺陷？**
...
相应地，可以构造$F$检验：
$$
F(\text{df}_{\text{SSR}}, \text{df}_{\text{SSE}}) = \frac{\text{MSR}}{\text{MSE}}
$$
...
从CHARLS数据中随机抽取$n = 500$个样本，考虑收入$x$对中老年抑郁水平$y$的影响。
...
# regression model
fit <- lm(cesd10 ~ income + educ, data = charlswh)
...
假定分类变量$edu$包含三个水平：小学以下（1）、中学（2）、大学及以上（3）
